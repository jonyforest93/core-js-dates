...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const divide = +end - +start;
  return divide / 1000 / 60 / 60 / 24 + 1;
}
...
function formatDate(date) {
  ...
  const minutes = entryDate.getUTCMinutes().toString().padStart(2, '0');
  const seconds = entryDate.getUTCSeconds().toString().padStart(2, '0');

  const period = hours >= 12 ? 'PM' : 'AM';
  const formattedHours = hours % 12 || 12;

  const result = `${month}/${day}/${year}, ${formattedHours}:${minutes}:${seconds} ${period}`;
...
function getCountWeekendsInMonth(month, year) {
  const firstDayOnMonth = new Date(year, month - 1, 1);
  const lastDayOnMonth = new Date(year, month, 0);

  let result = 0;

  for (
    let date = firstDayOnMonth;
    date <= lastDayOnMonth;
...
function getNextFridayThe13th(date) {
  const yearsToCheck = 100;
  let year = date.getFullYear();
  let month = date.getMonth();

  for (let i = 0; i < yearsToCheck; i += 1) {
    const nextFriday13 = new Date(year, month, 13);
